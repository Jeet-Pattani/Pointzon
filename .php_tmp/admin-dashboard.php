	...
	<div id="fh5co-wrapper">
		<div id="fh5co-page">
			...
			<div class="fh5co-hero fh5co-hero-2">
				...
				<div class="fh5co-cover fh5co-cover_2 text-center" data-stellar-background-ratio="0.5" style="background-image: url('https://images.pexels.com/photos/4065133/pexels-photo-4065133.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2');">
			...
			<div id="fh5co-contact" class="animate-box">
				<div class="container">
					<div class="desc animate-box">
						<h2 style="color: var(--stark);">Unapproved Campaigns</h2>
						<br><br>
						...
						<table class="table table-dark table-striped table-responsive" style="width:100vw;">
							<thead>
								<tr>
									<th scope="col">ID</th>
							...
							<tbody>
								<?php
								$cardQuery = "SELECT * FROM campaign WHERE approval=0";
								$execQuery = mysqli_query($con, $cardQuery);
								while ($card = mysqli_fetch_assoc($execQuery)) {
								?>
									<tr>
										<th scope="col"><?= $card['id'] ?></th>
										<td scope="col"><?= $card['b_name'] ?></td>
										<td scope="col"><?= $card['b_funding'] ?></td>
										<td scope="col"><?= $card['b_type'] ?></td>
										<td scope="col"><?= $card['b_email'] ?></td>
										<td scope="col"><?= $card['b_website'] ?></td>
										<td scope="col"><?= $card['b_phoneno'] ?></td>
										<td>
											<p class="intro-para" style="width:600px;"><?= $card['b_intro'] ?></p><!--<button type="button" class="btn btn-info read-more">Read More</button>-->
										</td>
										<td scope="col"><?= $card['b_tagline'] ?></td>
										<td scope="col"><img style="width:100px;height:100px;" data-src=../business/<?= $card['b_logo'] ?> alt=... class=lozad loading=lazy></td>
										<td scope="col">
											<form method="POST">
											<button type="submit" name="approve_campaign" value="<?= $card['id'] ?>" class="btn btn-success">Approve</button>
											</form>
										</td>
...
if(isset($_POST['approve_campaign'])){
    ...
    $campaign_id = $_POST['approve_campaign'];
    $query = "UPDATE campaign SET approval=1 WHERE id = '$campaign_id' LIMIT 1";
    ...
    echo "DB Error. Please Try after sometime...";
}
?>
